...
export default function ProgramsPage() {
  return (
    <>
      ...
      <section className="mx-auto max-w-4xl px-6 pb-28 pt-16 sm:px-12">
        {categories.map((category, i) => (
          <div
            key={category.title}
            id={category.slug}
            className={i > 0 ? "mt-20 scroll-mt-24" : "scroll-mt-24"}
          >
            <Reveal>
              <div className="bg-primary px-6 py-4 sm:px-8">
            ...
            <div className="mt-2">
              {category.programs.map((program, j) => (
                <Reveal key={program.title} delay={Math.min(j, 4) * 0.05}>
                  <ProgramItem {...program} isFirst={j === 0} />
                </Reveal>
              ))}
            </div>
          </div>
